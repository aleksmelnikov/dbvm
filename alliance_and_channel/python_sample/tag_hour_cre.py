...
#       df = pd.read_sql('select * from T_DAT_FILL rownum < 2', con, index_col='CDATE')
        ...
        query = ("""
               DROP TABLE T_DAT_HOURSUMMARY
               """)
        cur.execute(query)
        common.debugPrint("Drop table ..")

        idx = 0
        test = " CREATE TABLE T_DAT_HOURSUMMARY AS SELECT TO_DATE(TO_CHAR(CDATE,'YYYY-MM-DD HH24'),'YYYY-MM-DD HH24')  CDATE"
        print(test)
        for col in df.columns:
            test = test + "\n," + 'trunc(avg(' + col + ')) ' + col
        query = test + "\n from T_DAT_DCOR GROUP BY TO_CHAR(CDATE,'YYYY-MM-DD HH24')"
        cur.execute(query)
        con.commit()
        #print(table)
#
