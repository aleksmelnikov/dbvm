...
isql -s $HOST -u $USER -p $PASSWORD -port $PORT -f checkpoint.sql
sleep 1
java bmt_update $THREAD $START $RECORDS $UNIT $THRESHOLD $CAP $HOST $USER $PASSWORD $OPTION
START=`expr $START + $RECORDS`
done
...
isql -s $HOST -u $USER -p $PASSWORD -port $PORT -f checkpoint.sql
sleep 1
java bmt_delete $THREAD $START $RECORDS $UNIT $THRESHOLD $CAP $HOST $USER $PASSWORD $OPTION
START=`expr $START + $RECORDS`
done
