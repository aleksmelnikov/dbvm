...
# established over SSL.
#
# Usage: bash script/ssl_test_with_project_openssl.sh
#
...
echo "=============================================================="
echo "SSL TEST : PROJECT OpenSSL (bundled from target/dep/lib or lib64, via dbenv.sh --with-libs)"
...
# (OpenSSL 1.0.x) or target/dep/lib64 (OpenSSL 3.x).
BUNDLED_SSL="${ROOT_DIR}/target/dep/lib64/libssl.so"
if [ ! -e "${BUNDLED_SSL}" ]; then
    BUNDLED_SSL="${ROOT_DIR}/target/dep/lib/libssl.so"
fi
if [ ! -e "${BUNDLED_SSL}" ]; then
    echo "[FAIL] bundled library not found (looked in target/dep/lib64 and target/dep/lib)" >&2
    exit 1
fi
